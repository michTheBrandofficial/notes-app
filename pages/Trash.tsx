...
import { Deleted, SwipeGesture, TrashFallback } from '@/components/ui';
...
import { reaction } from 'nixix/primitives';
import { Button, HStack, Paragragh, VStack } from 'nixix/view-components';

class Trash extends Component {
  constructor() {
    super()
    this.setupTrashDbUpdate()
  }

  disabledState = Trash.State<boolean>(false)

  setupTrashDbUpdate() {
    reaction(() => {
      this.disabledState.set(!Boolean(trashStore.length));
    }, [trashStore]);
  }
  ...
  async jsx(): Promise<someView> {
    return (
      <SwipeGesture on:swiperight={this.goHome}>
        <VStack
          className={
            ...
            <HStack className="flex justify-between items-start w-full px-3 h-fit lg:justify-normal ">
              ...
              <VStack className="space-y-4 w-full h-fit">
                ...
                <TrashButton
                  ...
                  disabled={this.disabledState.get()}
                >
                  Empty trash now
                </TrashButton>
              </VStack>
